...
int main(int argc, char* argv[]) {
    ...
    convert_bgrhsv calc;
    uchar hsv[convert_bgrhsv::channel];
    calc.toHsvCV(convert_bgrhsv::channel, hsv, 165, 107, 72);
    ...
    calc.toHsvCV(convert_bgrhsv::channel, hsv, 158, 104, 74);
    cout << "bgr: 158, 104, 74" << endl;
    cout << "h:" << static_cast<unsigned>(hsv[0]) << " s:" << static_cast<unsigned>(hsv[1]) << " v:" << static_cast<unsigned>(hsv[2]) << endl;
    assert(hsv[0]==109); assert(hsv[1]==136); assert(hsv[2]==158);
    cout << endl;

    calc.toHsvCV(convert_bgrhsv::channel, hsv, 150, 91, 59);
    ...
    double hsv_d[convert_bgrhsv::channel];

    calc.toHsv(convert_bgrhsv::channel, hsv_d, 165, 107, 72); // html color picker 217°, 56%, 65%
    cout << "bgr: 165, 107, 72" << endl;
    cout << "h:" << hsv_d[0] << " s:" << hsv_d[1] << " v:" << hsv_d[2] << endl;
    cout << endl;

    calc.toHsv(convert_bgrhsv::channel, hsv_d, 150, 91, 59); // 219°, 61%, 59%
    cout << "bgr: 150, 91, 59" << endl;
    cout << "h:" << hsv_d[0] << " s:" << hsv_d[1] << " v:" << hsv_d[2] << endl;
     ...
     */
    uchar bgr[convert_bgrhsv::channel];
    calc.toBgr(convert_bgrhsv::channel, bgr, 217.419, 56.3636, 65.7059);
    ...
    calc.toBgr(convert_bgrhsv::channel, bgr, 218.901, 60.6667, 58.8235);
    ...
    uchar hsv_a[convert_bgrhsv::channel];
    calc.toHsvCV(convert_bgrhsv::channel, hsv_a, 142, 84, 49); 
    uchar hsv_b[convert_bgrhsv::channel];
    calc.toHsvCV(convert_bgrhsv::channel, hsv_b, 155, 98, 66);
